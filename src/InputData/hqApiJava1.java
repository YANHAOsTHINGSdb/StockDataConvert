...
public class hqApiJava1
{
	...
	static TdxHqLibrary TdxHqLibrary1;
...
//			boolean1=TdxHqLibrary1.TdxHq_GetIndexBars((byte)4,(byte)1,"1A0001",(short) 0, Count2, Result, ErrInfo);
//			if (!boolean1)
//			{
//				System.out.println(Native.toString(ErrInfo, "GBK"));
//				return;
...
//		String[] Zqdm={"399001","000001"};
	...
	public static boolean getGetSecurityList(byte Market, short start, ShortByReference Count, byte[] Result, byte[] ErrInfo) {
		...
		// 获取市场内指定范围内的所有证券代码
		// boolean  TdxHq_GetSecurityList(byte Market, short Start, ShortByReference  Count, byte[] Result, byte[] ErrInfo);
		boolean1= TdxHqLibrary1.TdxHq_GetSecurityList(Market, start, Count, Result, ErrInfo);
		if (!boolean1)
		{
			System.out.println(Native.toString(ErrInfo, "GBK"));
			return false;
	...
	public static boolean getConnect(String IP, int Port, byte[] Result, byte[] ErrInfo) {
		...
		TdxHqLibrary1 = (TdxHqLibrary)Native.loadLibrary("TdxHqApi1",TdxHqLibrary.class);
		return TdxHqLibrary1.TdxHq_Connect(IP, Port, Result, ErrInfo);
	}
}
