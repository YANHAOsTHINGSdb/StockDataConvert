...
public class InputDataUtil除权 extends InputDataUtil爸爸 {
	 ...
	 * @return
	 */
	public static boolean 取得除权数据和最后交易日(List<String> 除权数据list, String[] s最后交易日期, String[] s除权原始文件全路径) {
		if (!判断下载的除权文件是否有效(s除权原始文件全路径[0])) {
			// 除权文件下载
			return 取得除权数据和最后交易日_byDownload(除权数据list, s最后交易日期, s除权原始文件全路径);
		} else {
			return 取得除权数据和最后交易日_byFile(除权数据list, s最后交易日期, s除权原始文件全路径);
		}
	}

	protected static boolean 判断下载的除权文件是否有效(String s文件全路径) {
	...
	private static boolean 取得除权数据和最后交易日_byFile(List<String> 除权数据list, String[] s最后交易日期, String[] s除权原始文件全路径) {
		List<String> 除权数据list_local = InputDataUtil除权.getFileContext2(s除权原始文件全路径[0]);
		除权数据list.addAll(除权数据list_local);
		return true;
	}

	public static boolean 取得除权数据和最后交易日_byDownload(List<String> 除权数据list, String[] s最后交易日期, String[] s除权原始文件全路径) {
		byte[] Result = new byte[65535];
		byte[] ErrInfo = new byte[256];
		//		byte[] Market = { 0, 1 };
		//		byte[] Zqdm = null;
